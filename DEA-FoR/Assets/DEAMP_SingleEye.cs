...
public class DEAMP_SingleEye : MonoBehaviour
{
    ...
    [Range(1.00f, MAX_SIGMA)]
    public float sigma2 = 1.0f;
    [Range(1.00f, 8.0f)]
    public float sigma3 = 1.0f;
    ...
    [Range(0.0f, 45.0f)]
    public float e1 = 10.0f;
    [Range(0.0f, 45.0f)]
    public float e2 = 22.5f;
    [Range(45.0f, 45.0f)]
    public float e3 = 45.0f;

    float fov_deg = 20.0f;
    float medium_deg = 45.0f;
    float peri_deg = 90.0f;
    ...
    void incF(float d)
    {
        if ((mode == "dom") || (mode == "nondom" && !isDom())) { 
            sigma1 += d;
            if (sigma1 > MAX_SIGMA) sigma1 = MAX_SIGMA;
        }
    }
    ...
    void incM(float d)
    {
        ...
        {
            sigma2 += d;
            if (sigma2 > MAX_SIGMA) sigma2 = MAX_SIGMA;
        }
    }
    ...
    void incP(float d)
    {
        ...
        {
            sigma3 += d;
            if (sigma3 > MAX_SIGMA) sigma3 = MAX_SIGMA;
        }
    }
    ...
    {
        int idx = 0;
        DispText(idx++, sigma1, "¦Ò1");
        DispText(idx++, sigma2, "¦Ò2");
        DispText(idx++, sigma3, "¦Ò3");
